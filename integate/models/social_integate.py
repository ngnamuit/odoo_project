...
class SocialIntegrate(models.Model):
    ...
    def create_lead_contact_and_activities(self, vals={}):
        ...
        #region for lead
        lead = self.env['crm.lead'].search([('post_id', '=', str(vals['post_id']))])
        if not lead:
            lead_vals = {
                'first_name' : vals.get('first_name'),
                'last_name'  : vals.get('last_name'),
            ...
            if response.status_code == 200:
                ...
                if datas: # I like to write this code although It's not alive, Will be better haha
                    for row in datas:
                        ...
                        # define some variable to put into vals
                        ...
                        message   = row.get('message') or False
                    ...
                    logging.info(f"[GET_ACTIONS]- Can not found data {r}")
